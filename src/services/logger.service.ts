...
export class LoggerService {
  ...
  log(message: string) {
    // console.log("DEBUG - Logging message:", message); // Verifica en la consola del servidor
    this.logger.info(message);
    this.logStream.next(message); // Asegúrate de que esta línea se ejecute
  }

  error(message: string) {
    this.logger.error(message);
    this.logStream.next(`ERROR: ${message}`);
  }

  private initializeLogCleanup() {
    const interval = 1000 * 60 * 60 * 6; // cada 6 horas

    setInterval(() => {
      fs.writeFile(this.logFilePath, '', err => {
        if (err) console.error('Error limpiando el archivo de log:', err?.message);
        else console.log('🧹 Archivo de logs limpiado');
      });
    }, interval);
  }
}
