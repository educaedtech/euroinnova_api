...
import {GenInterface, ShopifyService, SyncResults} from '../services/shopify.service';
...
export class CreditosController {
  ...
  @post('/creditos/sync-to-shopify/{merchant_id}')
  async syncronizeCredits(
    @param.path.number('merchant_id') merchantId: number,
    @requestBody() data: any): Promise<{
    ...
    try {
      ...
      const creditosData = await this.creditosProductosRepository.find() as GenInterface[];
      ...
      const syncResult = await this.shopifyService.syncronizeCredits(creditosData, this.creditosProductosRepository, merchantId);
